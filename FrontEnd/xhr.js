...
console.log("A random number has been generated for WebSoket:", sn);
...
socket.onclose = async (event) => {
    console.error(event);
    window.alert('No connection with the Django Server! Check the server status!');
    console.log('No connection with the Django Server! Check the server status!');
};
...
socket.onmessage = function(event) {
    ...
    if ('UserList' in data) {
        ...
        if (userLoggedId) {
            ...
            console.log('current user is not selected!');
            printUsers(data);
        };
    };
...
function printUsers(data) {
    ...
    for (let key in data) {
        ...
        <td><button onclick="userLogged(${key})">choose</button></td>
...
function printUserCard(item) {
    ...
    <div class="div">
        ...
        <button onclick="editAvatar(${item.id})">send</button>
        <p>ID: ${item.id}</p>
        <p>Имя: ${item.name} <button onclick="changeUserName(${item.id})">update</button></p>
        <p>Chat room: ${room}</p>
...
async function editAvatar(userId) {
    ...
    if (fileField.files[0]) {
        ...
        try {
            ...
            console.log('User avatar saved:', JSON.stringify(result));
        } catch (error) {
            console.log('Ooops!!!');
        ...
        document.querySelector('#message').innerText = '!!! choose a file';
    };
};
...
function changeUserName(userId) {
    ...
    console.log('A request has been sent to the server to change the name to:', name);
};
...
function printRooms(data) {
    ...
    for (let key in data) {
        ...
        <td><button onclick="editRoom(${key})">update</button></td>
        <td><button onclick="selectRoom(${key})">connect</button></td></tr>`;
        list = list + newString;
    };
    list = '<table>' + list + '</table><br>'
    list = list + `<input type="text" id="input_room" name="name_new_room" size="22" placeholder="Enter the new room name"><br>`
    list = list + `<button class="btn btn_new_room">Create room</button>`
...
function editRoom(id) {
    ...
    console.log('A request has been sent to the server to change the room name to:', name);
};
...
function printChat(data) {
    ...
    <input type="text" id="input_message" name="input_message" size="22" placeholder="Print here..."><br>
